...
import { type PlannerData, type YearlyGoal, generateId } from "@/lib/planner-store"
...
export function YearlyWidget({ data, onUpdate }: YearlyWidgetProps) {
  ...
  const addGoal = () => {
    ...
    onUpdate({
      ...data,
      yearlyGoals: [...data.yearlyGoals, goal],
    })
    setNewText("")
    setIsAdding(false)
  }
  ...
  const toggleGoal = (goalId: string) => {
    onUpdate({
      ...data,
      yearlyGoals: data.yearlyGoals.map((g) =>
  ...
  const removeGoal = (goalId: string) => {
    onUpdate({
      ...data,
      yearlyGoals: data.yearlyGoals.filter((g) => g.id !== goalId),
    })
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {data.yearlyGoals.length > 0 && (
        <div className="text-xs text-muted-foreground">
          {completedCount}/{data.yearlyGoals.length} completed
        </div>
      )}
